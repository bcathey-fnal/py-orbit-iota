...
# Added - nilanjan@uchicago.edu, 10/18/21
class DipedgeTEAPOT(NodeTEAPOT):
    """
    Dipedge TEAPOT element.
    """
    def __init__(self, name = "dipedge no name"):
        """
        Constructor. Creates the Dipedge TEAPOT element .
        """
        NodeTEAPOT.__init__(self,name)
        self.setType("dipedge teapot")
        self.addParam("h", 0.0)
        self.addParam("e1", 0.0)
        self.addParam("fint", 0.0)
        self.addParam("hgap", 0.0)

    def track(self, paramsDict):
        """
        The Dipedge TEAPOT class implementation of the AccNodeBunchTracker class track(probe) method.
        """
        index = self.getActivePartIndex()
        bunch = paramsDict["bunch"]
        useCharge = 1
        ...
        TPB.dipedge(bunch,h,e1,fint,hgap)
