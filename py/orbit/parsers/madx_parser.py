...
class MADX_LattElement:
    ...
    def hasParameter(self, nameOfPar):
        """Method. Checks whether a parameter exists or not."""
...
class MADX_Parser:
    ...
    def __init__(self, verbosity=1):
        ...
        self.verbosity = verbosity # save the verbosity
    ...
    def collect_madx_lines(self, fileName, madFilePath):
        ...
        for str in fl:
            ...
            ignore_word = [
                word for word in self.__ingnoreWords if word in str0 and str0.lstrip(word) != str0]
            if ignore_word:
                if self.verbosity:
                    print("The line starting with {} word found. line [{}] is ignored".format(
                        ignore_word, str0))
                str_local = ""
                continue
        ...
        for str_local in self._madxLines:
            ...
            if re.search(r'[\w]* *:.*', str_local):
                if(str_local.rfind("sequence") >= 0):
                    # is it possible to have more than two tokens here?
                    tokens = str_local.split(":")
                    self._sequencename = tokens[0]
                    tmp_str = ":".join(tokens[1:])
                    aux = [x.split("l=")[-1]
                           for x in tmp_str.split(",") if "l=" in x]
                    ...
                    aux = [x.split("refer=")[-1]
                           for x in tmp_str.split(",") if "refer=" in x]
                    if not aux:
                        elementRefer = "centre"
                    else:
                        elementRefer = aux[0]
                    ...
                    if loc_flag_elem:
                        if "at=" in str_local:
                            tokens = str_local.split(",")
                            # remove location from the string (is it necessary?)
                            tmp = ",".join([x for x in tokens if "at=" not in x])
                        else:
                            tmp = str_local
                        # elem can be defined directly at the location!
                        elem = self.parseElem(tmp)
            ...
            if(str_local.rfind("at=") >= 0):
                ...
                if "," in str_local.split(":")[0]:
                    tokens = str_local.split(",")
                    # elem name 100% is the first position, otherwise the user did a mistake
                    elem_name = tokens[0]
                    tmp = tokens[1:]
                    aux = [x.split("at=")[-1] for x in tmp if "at=" in x]
                    # Try to evaluate positions from known variables -nilanjan@uchicago.edu 08/08/2021
                    position = eval(aux[0], globals(), localValDict)
                else:
                    tokens = str_local.split(":")
                    elem_name = tokens[0]
                    tmp_str = "".join(tokens[1:])
                ...
                latt_elem = self.recalculateParameters(
                    latt_elem, localValDict)
            ...
            if(str_local.rfind("endsequence") >= 0):
                ...
                endlength = float(self._sequencelength) - (float(self._sequencelist[-1].getParameter(
                    "position")) + 0.5*float(self._sequencelist[-1].getParameter("l")))
                if(endlength < -1e-10):
                    if self.verbosity:
...
# -------------------------------------------------------------------
    ...
    def parseParameters(self, line_init, lattElem):
        ...
        if "{" in line_init and "}" in line_init:
            ...
            line_init = "".join(
                [x if "!" not in x else "!" for x in line_init.split("{")])
            line_init = ",".join(
                [x for x in line_init.split(",") if "!" not in x])

            if "aperture" in line_tmp.lower():
                tmp = [s for s in line_tmp.split(
                    ",") if "apertype" in s.lower()]
                if tmp:
                    apertype = tmp[0]
                else:
        ...
        # name+type are already parsed, aperture and knl/ksl are excluded
...
# -------------------------------------------------------------------
    ...
    def makeMultiPols(self, line_init):
        ...
        skews = []
        line_init = line_init.replace("}", "!{")
        line_init = "".join([x if "!" not in x else "!".join(
            x.split(",")) for x in line_init.split("{")])
        tokens = [",".join(x.rstrip("!").split("!")) for x in line_init.split(
            ",") if "!" in x and "aper" not in x]
